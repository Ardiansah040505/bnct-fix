...
DetectorConstruction::DetectorConstruction()
 : G4VUserDetectorConstruction(),
   ...
   fCheckOverlaps(true),
   fShieldThickness(120.*cm), // default thickness
   fMessenger(nullptr)
{
  DefineMaterials();
  ...
  auto& cmd = fMessenger->DeclareMethod("setShieldThickness",
                                        &DetectorConstruction::SetShieldThickness,
                                        "Set neutron shielding thickness");
  cmd.SetUnitCategory("Length");
  cmd.SetParameterName("thick", true);
  cmd.SetRange("thick>0.");
}
...
void DetectorConstruction::DefineMaterials()
{
  ...
  new G4Material("BoronEnriched", density, 1)->AddElement(elB10, 1.0);
}
...
void DetectorConstruction::ConstructVolumes()
{
  ...
  // Keep fWorldPV pointing to the placed world
  // (fWorldPV was created above when placing logicWorld into physical world)
  // nothing else to return here (Construct() will return fWorldPV)
}
